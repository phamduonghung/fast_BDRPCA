...
for test =0:3
    ...
    elseif test ==1
        load(fullfile(running_folder,'simulation\RAPID_test\Results_LSDe_BDeconv',sprintf('%s',nomfichier),'SSGoDec.mat'));
    ...
    elseif test==2
        ...
        load(fullfile(running_folder,'ISBI_2020\fast_BDRPCA-GitHub\Results',sprintf('%s',nomfichier),'fDRPCA.mat'));
...
%     CNR(test+1) = round(abs(mean(R11(:))-mean(R21(:)))/sqrt(std2(R11)^2+std2(R21)^2),3)
    ...
    if test==1
    export_fig(FigHandle0, ... % figure handle
                sprintf('%s/%s_SSGoDec.pdf', result_folder,nomfichier),... % name of output file without extension
                '-painters', ...      % renderer
                '-transparent', ...   % renderer
                '-pdf', ...         % file format
                '-r5000');             % resolution in dpi 
...
boxplot([CR1(:,1),CR1(:,2),CR1(:,3),CR1(:,4)],'Notch','on','Labels',{'SVD','GoDec','BD-RPCA','fast BD-RPCA'})
